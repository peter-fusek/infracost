...
export default defineEventHandler(async (event) => {
  ...
  const old = await useDB().select().from(projects).where(eq(projects.slug, 'instarea'))
  if (!old.length) {
  ...
  const existing = await useDB().select().from(projects).where(eq(projects.slug, 'instarea.com'))
  if (existing.length) {
    return { status: 'error', message: 'Project "instarea.com" already exists — manual resolution needed' }
  }

  // 3. Rename the slug
  await useDB().update(projects).set({ slug: 'instarea.com', name: 'Instarea.com' }).where(eq(projects.id, oldProject.id))
